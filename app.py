...
if page == '📤 Upload / Sync':
    ...
    if engine:
        ...
        else: st.info('No uploads logged yet.')columns if c.startswith("__")], errors="ignore")
        ...
        def history_modal():
            ...
            for _, row in hist_df.iterrows():
                with st.container(border=True):
                    ...
                    with c2:
                        if st.button("✏️ Edit", key=f"edit_{row['ECN']}_{row['Field']}_{row['Start']}"):
                            st.session_state["hist_edit_id"] = {
                                "ecn": row["ECN"], "field": row["Field"],
                                "start": row["Start"], "value": row["Value"],
                                "end": row["End"]
                            }
                            st.rerun()
                    with c3:
                        if st.button("🗑️ Delete", key=f"del_{row['ECN']}_{row['Field']}_{row['Start']}"):
                            st.session_state["hist_del_id"] = {
                                "ecn": row["ECN"], "field": row["Field"], "start": row["Start"]
                            }
                            st.rerun()

            # ─── EDIT MODAL (nested dialog via session state) ─────────────
            if st.session_state.get("hist_edit_id"):
                edit_info = st.session_state["hist_edit_id"]
                st.divider()
                st.subheader("✏️ Edit Record")
                with st.form(key="hist_edit_form"):
                    nc1, nc2, nc3 = st.columns(3)
                    with nc1:
                        new_val = st.text_input("Value", value=edit_info["value"])
                    with nc2:
                        new_start = st.text_input("Start Date", value=edit_info["start"])
                    with nc3:
                        new_end = st.text_input("End Date", value=edit_info["end"])

                    with engine.connect() as conn:
                        rec = conn.execute(text("""
                            SELECT id FROM history 
                            WHERE ecn = :ecn AND field = :field AND start_date = :start
                        """), {"ecn": edit_info["ecn"], "field": edit_info["field"], "start": edit_info["start"]}).fetchone()
                    ...
                    with c1:
                        ...
                        if submitted:
                            if rec:
                                ...
                                if ok:
                                    st.success(msg)
                                    del st.session_state["hist_edit_id"]
                                    st.rerun()
                                else:
                                    st.error(msg)
                    with c2:
            ...
            # ─── DELETE CONFIRMATION ──────────────────────────────────────
            if st.session_state.get("hist_del_id"):
                del_info = st.session_state["hist_del_id"]
                st.divider()
                    ...
                    if st.button("✅ Yes, Delete", type="primary"):
                        with engine.connect() as conn:
                            rec = conn.execute(text("""
                                SELECT id FROM history 
                                WHERE ecn = :ecn AND field = :field AND start_date = :start
                            """), {"ecn": del_info["ecn"], "field": del_info["field"], "start": del_info["start"]}).fetchone()
                        if rec:
                            ok, msg = delete_history_record(rec[0])
                            if ok:
                                st.success(msg)
                                del st.session_state["hist_del_id"]
                                st.rerun()
                            else:
                                st.error(msg)
                with c2:
